...
**Cancelled-positions breakdown (per month, under the region chart).** Below the bar chart the GEO tab shows "Отменённые заказы — позиции, от которых отказались": cancelled line items grouped by product (expandable into per-color/variant rows). Needs `include=products` (line items) — so live months fetch `include=shipping,products` and the backfill does too. `lib/geo.aggregateCancelled(allMonthOrders)` → `{ orders, revenue, items:[{ name, qty, orders(=cancelled distinct orders), total_orders(=ALL-status distinct orders w/ this position), pct(=cancelled/total orders), revenue, variants:[{label,sku,qty,orders,total_orders,pct,revenue}] }] }`. Distinct orders via Sets (one order may hold several colors of one product — don't double-count line items). `mergeCancelled([...])` combines months for the «Общая» view (months don't overlap → counts add, pct recomputed). UI columns: Кол-во (отказов qty + % small) · Заказов (total) · Сумма. The breakdown is **per month**: historical months come from the snapshot (`months[ym].cancelled`), current+previous are live; «Общая» = `overall.cancelled`.
...
1. **`scripts/geo-backfill.js`** paginates the WHOLE history ONCE, slowly (sequential, ~1.1s/page → ~54 req/min, ~110s, `include=shipping,products`), and writes per-month region aggregates **+ the cancelled-positions breakdown** (`months[ym].cancelled`, via `aggregateCancelled`) to **`data/geo-snapshot.json`** (committed to the repo). Snapshot is ~150KB (was ~30KB before the cancelled detail).
2. **`/api/geo`** reads the snapshot (historical months, immutable enough) and re-fetches only **current + previous month live** (~20 req, ~1.7s), overlaying them on top. `lib/geo.buildGeoResponse` merges + computes the **overall** = sum across all months (regions and `overall.cancelled`).
3. To refresh older months (incl. their cancelled breakdown): re-run `node scripts/geo-backfill.js`, commit `data/geo-snapshot.json`, redeploy.
...
`/api/geo` returns `{ updated_at, took_ms, snapshot_generated_at, overall:{label,total_orders,total_cancelled,total_revenue,regions:[{name,orders,cancelled,revenue,pct}],cancelled:{orders,revenue,items}}, months:[{ym,label,...,regions,cancelled:{orders,revenue,items}}] }`. Regions sorted by `orders` desc. UI: dropdown «Общая» + each month, horizontal bar chart + cancelled-positions table below.
